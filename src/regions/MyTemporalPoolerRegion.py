...
# modified union temporal pooler version from own repo
from algorithms.union_temporal_pooler import UnionTemporalPooler
...
def _buildArgs(poolerClass, self=None, kwargs={}):
  ...
  init = MyTemporalPoolerRegion.__init__
...
class MyTemporalPoolerRegion(PyRegion):

  """
  MyTemporalPoolerRegion is a copy of NUPICs TemporalPoolerRegion
  with light modifications for serialization and minor details.

  --> All documentation left from TemporalPoolerRegion.
  ...
  def getAlgorithmInstance(self):
    """
    :returns: instance of the underlying Pooler Instance
    """
    return self._pooler
  ...
  def getBaseSpec(cls):
    ...
    """
    spec = dict(
      description=MyTemporalPoolerRegion.__doc__,
      singleNodeOnly=True,
      inputs=dict(
        activeCells=dict(
  ...
  def writeToProto(self, proto):
    ...
    :param proto: MyTemporalPoolerRegion capnproto object
    """

    proto.poolerType = self._poolerType
    proto.learningMode = bool(self.learningMode)
    proto.inferenceMode = bool(self.inferenceMode)

    proto.inputWidth = int(self._inputWidth)
    proto.columnCount = int(self._columnCount)
    proto.historyLength = int(self._historyLength)
    proto.minHistory = int(self._minHistory)

    self._pooler.write(proto.pooler)
  ...
  def readFromProto(cls, proto):
    ...
    instance = cls(proto.columnCount, proto.inputWidth,
                   proto.historyLength, proto.minHistory, proto.poolerType)


    instance.learningMode = proto.learningMode
    instance.inferenceMode = proto.inferenceMode

    instance._pooler = _getPoolerClass(proto.poolerType).read(proto.pooler)
