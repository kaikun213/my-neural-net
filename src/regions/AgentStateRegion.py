...
class AgentStateRegion(PyRegion):
  """
  ...
  def getSpec(cls):
    ...
    """
    spec = dict(
      description=AgentStateRegion.__doc__,
      singleNodeOnly=True,
      inputs=dict(
        activeColumns=dict(
        ...
        apicalInputD1=dict(
          description="An array of 0's and 1's representing top down input."
                      " The input will be provided to apical dendrites from D1(t-1).",
          dataType="Real32",
          count=0,
          required=False,
          regionLevel=True,
        ...
        apicalInputD2=dict(
          description="An array of 0's and 1's representing top down input."
                      " The input will be provided to apical dendrites from D2(t-1).",
          dataType="Real32",
          count=0,
          required=False,
          regionLevel=True,
  ...
  def compute(self, inputs, outputs):
    ...
    # Use prev. L2(t-1) and L4(t-1) input for basal (context interpreting)
    if "basalInput" in inputs:
      activeCellsExternalBasal = self.prevBasalInput
      self.prevBasalInput = inputs["basalInput"].nonzero()[0]
    else:
    ...
    print '~~~~~~~ L5 ~~~~~~~~'
    print 'Internal Basal connections', self.formInternalBasalConnections
  ...
  def writeToProto(self, proto):
    ...
    prevBasalInputProto = proto.init("prevBasalInput", len(self.prevBasalInput))
    for i, cell in enumerate(self.prevBasalInput):
        prevBasalInputProto[i] = int(cell)
  ...
  def readFromProto(cls, proto):
    ...
    instance.prevActiveCells = np.array(proto.prevBasalInput, dtype='uint32')
