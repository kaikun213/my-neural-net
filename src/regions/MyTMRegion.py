...
def _buildArgs(f, self=None, kwargs={}):
  ...
  Assigns all arguments to the function as instance variables of MyTMRegion.
...
class MyTMRegion(PyRegion):

  """
  MyTMRegion is designed to implement the temporal memory compute for a given
  HTM level.
  ...
  def __init__(self,
               ...
               # need to add them anywhere in MyTMRegion, unless you need to do
    ...
    # These calls whittle down kwargs and create instance variables of MyTMRegion
  ...
  def compute(self, inputs, outputs):
    """
    Run one iteration of :class:`~nupic.regions.tm_region.MyTMRegion` compute,
  ...
  def _compute(self, inputs, outputs):
    """
    Run one iteration of MyTMRegion's compute
    """
    ...
    if self.computePredictedActiveCellIndices:
      ...
      # Added: output predictions
      outputs["predictedCells"][:] = 0
      outputs["predictedCells"][predictedIndices] = 1

    # Temporary debug output
    print '~~~~~~~ L3/L2 ~~~~~~~~'
    print 'BottomUpIn', buInputVector.nonzero()[0]
    print 'Active cells', activeIndices
    print 'Predicted cells', predictedIndices
    print '~~~~~~~~~~~~~~~~~~~~~~~~~'
  ...
  def getBaseSpec(cls):
    ...
    :returns: (dict) the base Spec for MyTMRegion.
    """
    spec = dict(
      description=MyTMRegion.__doc__,
      singleNodeOnly=True,
      inputs=dict(
        bottomUpIn=dict(
      ...
      outputs=dict(
        ...
        predictedCells=dict(
          description="The cells that are predicted",
          dataType='Real32',
          count=0,
          regionLevel=True,
          isDefaultOutput=False),
  ...
  def setAlgorithmInstance(self, instance):
    """
    :set instance of the underlying
              :class:`~nupic.algorithms.temporal_memory.TemporalMemory`
              algorithm object.
    """
    self._tfdr = instance;
