...
def _buildArgs(f, self=None, kwargs={}):
  ...
  Assigns all arguments to the function as instance variables of MySPRegion.
  ...
  # TODO: where does numberOfCols come into MySPRegion?
  ourArgNames += [
    'numberOfCols',
  ]
...
class MySPRegion(PyRegion):
  """
  MySPRegion is a copy of SPRegion with minimal added functionality.
  -> All documentation is from original SPRegion.

  It is designed to implement the spatial pooler compute for a given
  HTM level.
  ...
  def __init__(self,
    ...
    # These calls whittle down kwargs and create instance variables of MySPRegion
  ...
  def _compute(self, inputs, outputs):
    """
    Run one iteration of MySPRegion's compute
    """
    ...
    outputs['anomalyScore'][:] = 0
      ...
      #print "MySPRegion input: ", buInputVector.nonzero()[0]
      #print "MySPRegion output: ", rfOutput.nonzero()[0]
  ...
  def getBaseSpec(cls):
    ...
    :returns: (dict) The base Spec for MySPRegion.
    """
    spec = dict(
      description=MySPRegion.__doc__,
      singleNodeOnly=True,
      inputs=dict(
        bottomUpIn=dict(
